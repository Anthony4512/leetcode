...
fun maxSumOfPairWithEqualSumOfDigits(nums: IntArray): Int {
    var currentNumString: String
    var currentSum: Int
    var currentDigit: Int
    val sumToNumberMap = mutableMapOf<Int,Int>()
    var result = -1

    for (num in nums) {
        currentNumString = "$num"
        currentSum = 0
        for (c in currentNumString) {
            currentDigit = c.digitToInt()
            currentSum += currentDigit
        }
        if (sumToNumberMap.containsKey(currentSum)) {
            result = max(result, sumToNumberMap[currentSum]!! + num)
        }

        sumToNumberMap[currentSum] = max(sumToNumberMap.getOrDefault(currentSum, 0), num)
    }
    return result
}
