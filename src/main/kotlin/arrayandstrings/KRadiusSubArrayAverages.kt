...
fun kRadiusSubArrayAverage(nums: IntArray, k: Int): LongArray {
    val averages = LongArray(nums.size) { _ -> -1 }
    if (nums.size <= k * 2) return averages

    val totalRadius = (k * 2) + 1
    ...
    while (right < nums.size) {
        val average: Long = ((prefixSum[right] - prefixSum[left] + nums[left]) / totalRadius).toLong()
        averages[middle] = average
